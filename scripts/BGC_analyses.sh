...
# via bgc (and outputs Fig 4, Table S5, and Fig S5) in
# Alexander et al.
...
number_of_matches=`wc -l Table_S5_outlying_marker_bed_format.bed | awk '{print $1}'`

# The bed file has coordinates in both kb and bp - we want bp
cut -d" " -f1,4,5 Table_S5_outlying_marker_bed_format.bed > temp 
mv temp Table_S5_outlying_marker_bed_format.bed
rm temp

# Outputting a list of genes found in the regions of interest (potential inversions)
for i in `seq 1 $number_of_matches`;
  do bedline=`head -n $i Table_S5_outlying_marker_bed_format.bed | tail -n 1`;
...
# Place Table_S5_positive_beta_SNPs_bed_format.bed into this folder
number_of_matches=`wc -l Table_S5_positive_beta_SNPs_bed_format.bed | awk '{print $1}'`

# Outputting a list of genes found in the regions of interest
for i in `seq 1 $number_of_matches`;
  do bedline=`head -n $i Table_S5_positive_beta_SNPs_bed_format.bed | tail -n 1`;
  echo $bedline > temp.bed;
  seqname=`echo $bedline | awk '{print $1}'`;
  seqtk subseq ../GCA_011421415.1_CUB_Patr_1.0_genomic.fna temp.bed > $seqname.$i.fa;
  magicblast -query ../GCA_013398625.1_ASM1339862v1_cds_from_genomic.fna -subject $seqname.$i.fa -perc_identity 99 -outfmt tabular -no_unaligned > $seqname.$i.matches.txt;
...
# Getting a summary of what genes were associated with what SNPs to add to Table S5
for i in `seq 1 $number_of_matches`; 
