...
# (For_Fig_S6.txt, MO_change.txt, PA_change.txt). It outputs Fig S6 and S8
...
ggsave("FigS6a_temp_correlation.pdf")

temprsq <- cor(na.omit(for_figs6$P_00to10),na.omit(for_figs6$P_78to14))^2

ggplot(for_figs6,mapping=aes(x=P_00to10,y=P_78to14)) + geom_hex() +
  theme_bw() + 
  geom_smooth(method = "lm",color="red") +
...
ggsave("FigS6b_precip_correlation.pdf")
...
ggsave("FigS8_MO_PA_temp_changes.pdf")
